...
class SubscriptionPurchaseService
{
    ...
    /**
     * Repeat delivery dates for the full subscription period.
     * If a template date is in the past, it shifts forward month-by-month until it's today or later,
     * then expands for the full period from there.
     *
     * Example: ["2026-08-20", "2026-09-09"] with today 2026-08-28 × 3 months
     *   → 2026-09-09, 2026-09-20, 2026-10-09, 2026-10-20, 2026-11-09, 2026-11-20
     *
    ...
    public function expandDeliveryScheduleFromMonthlyTemplate(array $monthlyTemplate, int $periodMonths): array
    {
        ...
        foreach ($monthlyTemplate as $date) {
            ...
            for ($i = 0; $i < $periodMonths; $i++) {
                $schedule[] = $base->copy()->addMonths($i)->format('Y-m-d');
            }
        }
    ...
    /**
     * Generate all subscription orders based on actual delivery schedule
     */
    protected function generateSubscriptionOrders(
        ...
        // Calculate total value of all items to distribute discount
        $monthItems = $this->distributeItemsForMonth($offer, $ordersPerMonth);
        $singleMonthItemsValue = 0;
        foreach ($monthItems as $orderItems) {
            foreach ($orderItems as $item) {
                $singleMonthItemsValue += $item['quantity'] * $item['unit_price'];
            }
        }
        $totalItemsValue = $singleMonthItemsValue * $periodMonths;

        $totalDeliveries = count($deliverySchedule);

        for ($i = 0; $i < $totalDeliveries; $i++) {
            $deliveryDate = $deliverySchedule[$i];
            $orderInMonth = ($i % $ordersPerMonth) + 1;
            $monthNumber = (int) floor($i / $ordersPerMonth) + 1;
            $itemsIndex = ($i % $ordersPerMonth);

            $itemsWithDiscounts = $this->applyProportionalDiscounts(
                $monthItems[$itemsIndex] ?? $monthItems[0],
